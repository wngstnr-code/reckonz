 ...
 *
 * ## The box is cropped to the ink, and the drawing is untouched
 *
 * It exits sideways rather than running to the bottom of its sheet, so the last
 * 600 units of the 1600 held nothing. That was not free. An absolutely
 * positioned box still contributes to the page's scrollable area, and 600 empty
 * units at this scale is most of a screen of white after the footer — the page
 * appeared to carry on past its own ending.
 *
 * The height is the ink's own extent plus half a stroke: `getBBox` puts the
 * last of it at 983, and 1010 clears the round cap. **Only the height changed.**
 * The scale comes from the width — 960 user units mapped to the full page — and
 * the origin is still 0,0, so every coordinate in the `d` lands exactly where
 * it did. Cropping the box moved nothing that is drawn in it.
 *
